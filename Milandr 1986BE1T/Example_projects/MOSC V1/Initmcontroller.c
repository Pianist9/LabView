...
{   ///----------------------------------Настройка тактовой частоты---------------------------------------------------------------|
	  ...
	  MDR_RST_CLK->PER_CLOCK  |= (1<<4);   //тактирование RCT_CLC
		//-----------------------------------ТАКТОВАЯ ЧАСТОТА -------------------------------------------
	  MDR_RST_CLK->PLL_CONTROL = 0x804; 	   //Регистр управления блоками умножения частоты  PPLCPUo = PPLCPUi*(PLLCPUMUL+1) 104/(404 max speed 16*[4+1]=80  16*[8+1]=144) ;
...
{   ///----------------------------------Настройка UART1--------------------------------------------------------------------------|
	  ...
	  MDR_RST_CLK->UART_CLOCK |= (1<<24);  //Регистр управления тактовой частотой UART 
		MDR_UART1->IBRD  = 0x0004;             //Целая часть коэффициента деления частоты для формирования тактового сигнала передачи данных. После сброса устанавливается в 0
		MDR_UART1->FBRD  = 0x0039 ;               //дробная 4/(16*115200) =2,17    2 | 0,17   (0,17*64)+0,5 = 11 =0X0B
...
{   ///----------------------------------Настройка UART2--------------------------------------------------------------------------|
	  ...
	  MDR_RST_CLK->UART_CLOCK |=(1<<25);  //Регистр управления тактовой частотой UART
		...
		MDR_UART2->FBRD  = 0x002E;            //дробная 4/(16*115200) =2,17    2 | 0,17   (0,17*64)+0,5 = 11 =0X0B
...
{   ///---------------------------------------  Настройка ПОРТА A  (аннотация)  --------------------------------------------------|
	  ...
	  MDR_PORTA->OE     = 0x2207;					//Режим работы контроллера. Направление передачи данных на выводах порта:1 – выход;0 – вход
...
{	  ///----------------------------------Настройка ПОРТА B -----------------------------------------------------------------------|
	  ...
	  MDR_RST_CLK->PER_CLOCK |=(1<<22);    //тактирование PORTD
		MDR_PORTB->OE     = 0x0;             //xxxxxx10
		MDR_PORTB->FUNC   = 0x0;      //UART2
		MDR_PORTB->ANALOG = 0x0;      //
		MDR_PORTB->PD     = 0x0;          //драйвер/сток
		MDR_PORTB->PWR    = 0x0;      //Макс фронт
		MDR_PORTB->GFEN   = 0x0;            //фильтрация импульсов до 10 нс
}   //____________________________________________________________________________________________________________________________|

void PORTC(void)
{	  ///----------------------------------Настройка ПОРТА C UART-------------------------------------------------------------------|
	
	  MDR_RST_CLK->PER_CLOCK |=(1<<23);    //тактирование PORTD
		MDR_PORTC->OE     = 0x0308;          //xxxxxx10
		MDR_PORTC->FUNC   = 0x0140;          //UART1
...
{	  ///----------------------------------Настройка ПОРТА D ножка PD0 PD1   UART2--------------------------------------------------|
	  ...
	  MDR_RST_CLK->PER_CLOCK |=(1<<24);    //тактирование PORTD
		MDR_PORTD->OE     = 0x00;             //xxxxxx10
		MDR_PORTD->FUNC   = 0xFF;      //UART2
		MDR_PORTD->ANALOG = 0xFF;      //
		MDR_PORTD->PD     = 0x00;          //драйвер/сток
		MDR_PORTD->PWR    = 0xFF;      //Макс фронт
		MDR_PORTD->GFEN   = 0x00;            //фильтрация импульсов до 10 нс
}   //____________________________________________________________________________________________________________________________|
...
{	  ///----------------------------------Настройка ПОРТА B -----------------------------------------------------------------------|
	  ...
	  MDR_RST_CLK->PER_CLOCK |=(1<<22);    //тактирование PORTD
		MDR_PORTE->OE     = 0x0;             //xxxxxx10
		MDR_PORTE->FUNC   = 0x0;      //UART2
		MDR_PORTE->ANALOG = 0x0;      //
		MDR_PORTE->PD     = 0x0;          //драйвер/сток
		MDR_PORTE->PWR    = 0x0;      //Макс фронт
		MDR_PORTE->GFEN   = 0x0;            //фильтрация импульсов до 10 нс
}   //____________________________________________________________________________________________________________________________|
...
{	  ///----------------------------------Настройка ПОРТА F ножка PF0 PF1   UART2--------------------------------------------------|
	  ...
	  MDR_RST_CLK->PER_CLOCK |=(1<<29);     //тактирование PORTF
		MDR_PORTF->OE     = 0x2;              //xxxxxx10
		MDR_PORTF->FUNC   = 0xFF;       //UART1
		MDR_PORTF->ANALOG = 0xFF;       //
		MDR_PORTF->PD     = 0x00;           //драйвер/сток
		MDR_PORTF->PWR    = 0xFF;       //Макс фронт
		MDR_PORTF->GFEN   = 0x00;             //фильтрация импульсов до 10 нс
}   //____________________________________________________________________________________________________________________________|
