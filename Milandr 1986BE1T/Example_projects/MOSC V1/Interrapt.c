...
//=-=-=-=-=-=-=-=-=-=-=-=Прерывание прием =-=-=-=-=-=-=-=-=-=-=-=-=-=-	
		if ((MDR_UART1->RIS & 0x10)== 0x10)
	      ...
	      RXU2byte = MDR_UART1->DR&0xFF;                                                                                                    								 
				bufferRXU2[iRXU2]=RXU2byte;                                                       //получаем символ ложим символ в буфер.
				if (bufferRXU2 [1] != 0x10){nU2=8;}					
				else if ((bufferRXU2 [1] == 0x10) && (iRXU2 == 6)) {nU2 = 9 + bufferRXU2[6];} 
				iRXU2++;                                                                          //наращиваем счетчик буфера	
				//RXU2_q = 1;		
	      if(iRXU2>=nU2)
        {RXU2_flag=1;iRXU2=0; UNAR=1;}
							...
							while (c < nU2)
							  {
								...
								if ((MDR_UART2->FR & 0x80)== 0X80) 
									{
									MDR_UART2->DR = massU2[c];    //передача    
									c++;												
									}
